...
describe("ByteArray", () => {
  ...
  test("should grow when data exceeds capacity", () => {
    ...
    expect(buf.buf.length).toBeGreaterThanOrEqual(11);
    expect(buf.buf.subarray(0, 11).toString()).toBe("hello world");
  });
  ...
  test("should append multiple buffers correctly", () => {
    ...
    expect(buf.buf.subarray(0, 6).toString()).toBe("abcdef");
  });
});
